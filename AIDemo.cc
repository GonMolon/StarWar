...
struct PLAYER_NAME : public Player {
	...
	bool up(Dir d) {
		return d == SLOW_UP   or d == UP   or d == FAST_UP;
	}
	bool down(Dir d) {
		return d == SLOW_DOWN or d == DOWN or d == FAST_DOWN;
	}
	...
	virtual void play () {
		...
		// Initialize a vector with all allowed directions.
			all_dirs = {UP, DEFAULT, DOWN, SLOW_UP, SLOW, SLOW_DOWN,
		...
		for(Starship_Id sid = begin(me()); sid != end(me()); ++sid) {
			...
			if(s.alive) { // if the starship is alive and
				...
				if (s.nb_miss > 0 and ahead.type == STARSHIP) {
					...
					if (p != me()) shoot(sid);
				} else { // if a bonus can be reached move there
